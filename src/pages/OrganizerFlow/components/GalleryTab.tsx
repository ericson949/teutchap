...
import { Cloud, Trash2, Eye, Download, FolderArchive, Loader2, Upload, X, User, Heart, Zap, Sparkles, ArrowLeft } from 'lucide-react'
...
export const GalleryTab = ({ 
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          photos.map((photo: any) => {
            ...
            return (
              <div 
                ...
                className="relative aspect-[3/4] rounded-2xl overflow-hidden glass border border-white/5 shadow-xl group cursor-pointer"
              >
                <img 
                ...
                {photoChallenge && (
                  <div className="absolute top-2 left-2 glass px-2 py-0.5 rounded-lg text-[7px] font-black uppercase tracking-wider text-primary-light flex items-center space-x-1 border border-white/10 z-20">
          ...
          <div className="glass rounded-[3rem] p-8 max-w-lg w-full border border-white/10 flex flex-col space-y-6 shadow-2xl relative overflow-hidden max-h-[90vh]">
            ...
            <div className="glass p-5 rounded-[2rem] border border-white/5 flex items-center justify-between space-x-4 relative overflow-hidden group">
              ...
              <button 
                ...
                className={`w-12 h-6 rounded-full transition-all relative shrink-0 ${organizerCompress ? 'bg-[#aa3bff] shadow-[0_0_15px_rgba(170,59,255,0.4)]' : 'bg-white/10'}`}
              >
      ...
      {/* Modale de Détail Premium Organisateur */}
        ...
        <div className="fixed inset-0 z-[100] bg-black/90 backdrop-blur-lg flex flex-col items-center justify-center p-4 animate-in fade-in duration-300">
          <div className="relative glass rounded-[2.5rem] p-6 max-w-sm w-full border border-white/10 flex flex-col space-y-6 shadow-2xl overflow-y-auto max-h-[90vh] scrollbar-none">
            ...
            <div className="space-y-4 pt-1">
              ...
              {/* Bouton de Suppression Organisateur */}
              <button 
                onClick={(e) => { setSelectedPhoto(null); handleDeletePhoto(selectedPhoto, e); }}
                className="w-full py-4 bg-red-500/10 hover:bg-red-500/20 text-red-400 border border-red-500/15 rounded-2xl font-black uppercase text-[10px] tracking-widest transition-all flex items-center justify-center space-x-2"
              >
                <Trash2 size={14} />
                <span>Supprimer du Cloud</span>
              </button>
            </div>
          </div>
        </div>,
      ...
      {/* Lightbox Plein Écran Premium Organisateur */}
      {selectedPhoto && isFullscreen && createPortal(
        ...
        >
          ...
          <div className="absolute inset-0 bg-cover bg-center blur-3xl opacity-30 pointer-events-none scale-110" style={{ backgroundImage: 'url(' + import.meta.env.VITE_SUPABASE_URL + '/storage/v1/object/public/events_photos/' + (selectedPhoto.url_original || selectedPhoto.url_thumb) + ')' }} />
